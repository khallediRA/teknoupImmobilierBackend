...
public class ImmobilierService {
    ...
    Immobilier save(ImmobilierPostRequest postRequest, City city) {
        return immobilierRepository.save(Immobilier.builder().adresse(postRequest.getAdresse())
                ...
                .price(postRequest.getPrice()).city(city).superficie(postRequest.getSuperficie()).build());
    }

    Immobilier updateImmobilier(Long immobilierId, City city, ImmobilierPostUpdateDto immobilierPostUpdateDto) {

        Immobilier oldImmobilier = immobilierRepository.findById(immobilierId).orElseThrow(
                () -> new ImmobilierNotFoundException("immobilier Not Found with id" + immobilierId.toString()));
        return immobilierRepository
                .save(Immobilier.builder().id(oldImmobilier.getId()).adresse(immobilierPostUpdateDto.getAdresse())
                        .city(city).description(immobilierPostUpdateDto.getDescription())
                        .price(immobilierPostUpdateDto.getPrice()).imageURL(immobilierPostUpdateDto.getImageUrl())
